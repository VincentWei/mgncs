...
/*
 ** $Id: flat_listview.c 303 2009-07-03 02:24:38Z xwyan $
 **
 ** The flat renderer implementation of mListView control.
 **
...
static void flat_drawFold (mListView *self, HDC hdc,
        const RECT* pRect, DWORD color, HICON icon, int status, int next)
{
    ...
    if (status & LFRDR_TREE_WITHICON) {
        char szValue[255];
        HICON hFoldIcon   =
            (HICON)RetrieveRes (_get_icon_file("flat", (char*)SYSICON_TREEFOLD, szValue));
        HICON hUnFoldIcon =
            (HICON)RetrieveRes (_get_icon_file("flat", (char*)SYSICON_TREEUNFOLD, szValue));
...
static void flat_drawHeader (mListView *self, HDC hdc, const RECT *pRect, DWORD color, int flag)
{
    DWORD c;
    if(color ==0)
    ...
    self->renderer->draw3dbox(self, hdc, pRect, c, flag | NCSRF_FILL | 0x80000000);
}

static void flat_listview_class_init (mListViewRenderer* rdr)
{
	flat_widget_renderer.class_init((mWidgetRenderer*)rdr);
	rdr->drawFold = flat_drawFold;
	rdr->drawHeader = flat_drawHeader;
}

mListViewRenderer flat_listview_renderer = {
    "",
	flat_listview_class_init,
	(mItemViewRenderer*)(void *)&flat_widget_renderer
};
