...
static BOOL mWidget_callUserHandler( mWidget* self, void *handler, UINT message, WPARAM wParam, LPARAM lParam,
                LRESULT *pret)
{
    LRESULT ret = 0;
    switch(message)
...
static inline ht_key_t bind_info_to_key(HWND hwnd_listener, UINT message_id)
{
...
static MsgBindInfo* find_msgbind(HWND hwnd_listener, UINT message_id, BOOL bAutoCreate)
{
...
BOOL ncsBindEventsToMessage(mObject *sender, HWND hwnd_listener, UINT message, int* event_ids)
{
    BOOL bret = FALSE;
    MsgBindInfo * msgbind;
...
BOOL ncsBindEventToMessage(mObject* sender, HWND hwnd_listener, UINT message, int event_id)
{
...
void ncsRemoveBindMessage(HWND hwnd_listener, UINT message, mObject* sender/*=NULL*/,int event_id/*=0*/)
{
    MsgBindInfo * msgbind;
    MSGBIND_LOCK();
