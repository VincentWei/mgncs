...
static LRESULT mTextEditor_wndProc(mTextEditor *self, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message) {
...
BOOL RegisterTextEditControl (void)
{
...
#else
    WndClass.iBkColor    = GetWindowElementPixelEx (HWND_NULL, HDC_SCREEN, WE_BGC_WINDOW);
#endif
    WndClass.WinProc     = mTextEditCtrlProc;

    init_textedit();

    if (RegisterWindowClass (&WndClass) != ERR_OK)
        return FALSE;
    ...
    return RegisterWindowClass (&WndClass) == ERR_OK;
}
